...
namespace HMI.Main.Symbols.SKILL
{
  partial class sDefault
  {
    ...
    private event EventHandler<HMI.Main.Symbols.SKILL.OUTPUTEventArgs> OUTPUT_Fired;

    private event EventHandler<HMI.Main.Symbols.SKILL.SKILL_OUTPUTEventArgs> SKILL_OUTPUT_Fired;
      ...
      switch(eventIndex)
      {
        ...
        case 0:
          if (OUTPUT_Fired != null)
          {
            try
            {
              OUTPUT_Fired(this, new HMI.Main.Symbols.SKILL.OUTPUTEventArgs(channelId, cookie, eventIndex));
            }
...
{4}","OUTPUT_Fired", this.GetType().Name, this.CATName, e.Message, e.StackTrace);
        ...
        case 1:
          if (SKILL_OUTPUT_Fired != null)
          {
            try
            {
              SKILL_OUTPUT_Fired(this, new HMI.Main.Symbols.SKILL.SKILL_OUTPUTEventArgs(channelId, cookie, eventIndex));
            }
            catch (System.Exception e)
            {
              NxtControl.Services.LoggingService.ErrorFormatted(@"In Event Callback for event:'{0}' Type:'{1}' CAT:'{2}' came exception:{3}
...
{4}","SKILL_OUTPUT_Fired", this.GetType().Name, this.CATName, e.Message, e.StackTrace);
    ...
    public bool FireEvent_INPUT(HMI.Main.Symbols.SKILL.INPUTEventArgs ea)
    {
    ...
    public bool FireEvent_SKILL_INPUT(HMI.Main.Symbols.SKILL.SKILL_INPUTEventArgs ea)
    {
      object[] _values_ = new object[1];
